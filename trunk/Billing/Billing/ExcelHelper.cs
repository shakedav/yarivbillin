...
namespace Billing
{
    class ExcelHelper
    {
        ...
        static String Path = ConfigurationSettings.AppSettings["excelFilePath"];
       ...
       OleDbConnection dbCon = new OleDbConnection(sConnection);
        ...
        public void SaveDataToExcel(DataRow row, string sheetName)
        {
            ...
            {
                dbCon.Open();
                query = string.Format("{0} {1} = {2}",query, row.Table.Columns[i].ColumnName, item.ToString());
                string command = string.Format("Update [{0}$] set {1};", sheetName, query);
                OleDbCommand dbCmd = new OleDbCommand(command, dbCon);
